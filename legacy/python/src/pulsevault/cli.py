...
from pulsevault.core.vault import EncryptedVault, VaultError, human_size, password_policy_code
...
def cmd_create(args: argparse.Namespace) -> int:
    ...
    # Policy checks may classify the secret, but stdout must never receive the secret
    # or any value dataflow-tainted from it (CodeQL: clear-text logging of sensitive data).
    # Emit only fixed literals chosen by reason code (control flow), never format(pw/err).
    _weak = password_policy_code(pw)
    if _weak == "too_short":
        print("Warning: Use at least 14 characters, or generate a secure key.")
    elif _weak == "common":
        print("Warning: Avoid common words, keyboard patterns, and obvious number runs.")
    elif _weak == "repetitive":
        print("Warning: Use a less repetitive password.")
    elif _weak == "low_variety":
        print("Warning: Use more variety, or make it at least 20 characters.")
    elif _weak is not None:
        print("Warning: Password does not meet recommended policy.")
